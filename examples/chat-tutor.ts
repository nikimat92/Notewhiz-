...
// Local interfaces for Prisma result shapes (full types generated by `prisma generate`)
interface PackFlashcard { question: string; answer: string }
interface PackGlossaryItem { term: string; translation: string; example: string | null }
interface TutorMessageRow { role: string; content: string }
...
export async function POST(req: NextRequest) {
  ...
  // Fetch all three in parallel — they are independent queries.
  const [summary, packFlashcards, packGlossary] = await Promise.all([
    conversation.summaryId
      ? prisma.summary.findUnique({ where: { id: conversation.summaryId } })
  ...
  if (packFlashcards.length > 0) {
    packLines.push(`\nFLASHCARDS:\n${(packFlashcards as PackFlashcard[]).map(c => `• ${c.question} → ${c.answer}`).join('\n')}`);
  }
  if (packGlossary.length > 0) {
    packLines.push(`\nGLOSSARY:\n${(packGlossary as PackGlossaryItem[]).map(g => `• ${g.term}: ${g.translation}`).join('\n')}`);
  }
...
Answer with: 🎯 Practice Challenge → 💭 Hints → ✅ Solution → 🌍 Real-World Application → 📈 Next Level`,
  ...
  const history = (conversation.messages as TutorMessageRow[])
